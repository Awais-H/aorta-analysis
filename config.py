...
IMAGE_EDGE_MARGIN_VOXELS = 1.0  # rule 2 (path_at_image_edge): a traced path running within one native voxel of the image boundary in its first SEED_DISTANCE_MM has its cross-sections truncated by the volume, so the 5 mm followability the PDF requires cannot be verified in the data; the annotators excluded case 20's superior-crop track for exactly that. Only faces where the native volume ends count (the crop keeps CROP_PAD_MM of padding elsewhere). Measured on all 25 cases, 13 Sep: of 147 kept branches only two run within two voxels of an image face, subject 20 patch 4 at 0.1 voxels (the track the annotators excluded; rejected) and subject 6 patch 5 at 1.1 voxels (the aortic lumen outside the mask at the inferior image edge; not caught at one voxel and left to rule 1), no reference. Anything from 1.1 to 2 voxels changes nothing else on the dev set; one voxel is the physical statement and stays
...
LUMEN_PROBE_MM = 5.0        # rule 1 terminal division: the bright lumens are counted in a slab this far beyond the inferior end face, the seed distance, where the two common iliacs have separated (subjects 22, 23: two 9 to 13 mm lumens; 19 to 21: one 17 to 20 mm lumen, the aorta continuing)
LUMEN_PROBE_LATERAL_FACTOR = 1.5  # rule 1 terminal division: lumens counted within 1.5 aortic radii of the extended axis, where both iliacs lie; a renal leaving sideways at the cut is further out or smaller
ILIAC_LUMEN_MIN_DIAMETER_MM = 6.0  # rule 1 terminal division: both lumens must be at least 6 mm across (adult common iliacs are 8 to 12 mm; the largest visceral daughters are under 6 mm at 5 mm out, and a lumbar is 2 to 3 mm)
ILIAC_LUMEN_RATIO_MIN = 0.5  # rule 1 terminal division: the second lumen is at least half the first: the two iliacs are alike, a big continuing aorta next to a small branch is not a division (subject 21: 17 and 4 mm)
ILIAC_CANDIDATE_MIN_DIAMETER_MM = 5.0  # rule 1 terminal division: a candidate on the inferior face of a segment that ends at its division with an origin this wide is one of the iliacs (the only vessels over 5 mm at the terminal aorta; the inferior mesenteric is 3 to 4 mm and leaves 3 to 4 cm higher, the median sacral 1 to 2 mm)
...
BLOB_INSCRIBED_RADIUS_MM = 7.0  # rule 4 blob_at_seed: the inscribed circle of the seed cross-section reaching 7 mm means the seed sits inside a bright region at least 14 mm across: no daughter lumen is that wide 5 mm from the aorta (the largest reference seed section is 9 mm across; celiac and SMA are under 10 mm; the iliacs are out of scope). Not 8: the section is sampled in a 16 mm window, so the inscribed radius of anything wider saturates at 7.9 to 8.1 and a threshold there is a coin flip (subject 22 patch 32 read 12.1 with one tracer variant and 7.98 with another). Fires on an intervertebral disc contact at a 105 HU threshold (subject 22) and on the heart and arch contacts of subject 25, on no reference
