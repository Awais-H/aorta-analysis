...
- All detection geometry (shell, wall layer, distance transform) is relative to the raw largest-component mask. The reported ostium is snapped to that mask's boundary.
...
- D (watershed seeded from those components through the full 15 mm shell) assigns every candidate voxel to exactly one branch, so two branches that touch further out stay separate. Those voxels feed D4 and D5.
...
- Radius sanity: also compute sqrt(area / pi) at the seed. If it disagrees with the inscribed-circle radius by more than 30%, use the smaller. A too-large radius usually means the cross-section grabbed adjacent bright tissue.
...
7. Noise. Wall patch below the organiser's minimum origin size once announced. Placeholder until then: 1.5 mm equivalent radius.
...
- Scorer: one-to-one matching via Hungarian assignment (scipy.optimize.linear_sum_assignment) on ostium distance with a cutoff. Report at 3, 5 and 8 mm cutoffs; 5 mm is the working value since the organisers have not specified one. Metrics per case and aggregated: precision, recall, F1, mean matched ostium distance, direction angle error, radius absolute error, seed-inside-branch.
...
5. Is the reference ostium annotated on the aortic wall surface, or at the lumen centre of the opening?
6. If a daughter's ostium sits within a few mm of a cropped end of the mask (subject 3, superior cut), is it included in the reference annotations?
7. Subjects 16 to 20 are 1.5 mm isotropic and visibly from a different source. What fraction of the hidden set is coarse-resolution, and were their references annotated at native resolution?
