...
Findings from the false-positive review (13 Sep, evening; the five large wall-hugging survivors on subjects 21 and 22, the four candidates at the cut faces, and the reference package read against them):

- **Three of the five are one vessel touching in passing.** Subject 21's patches 4, 5 and 10 (origin diameters 3.8, 8.1, 8.3 mm; 4 mm apart at the posterior-left wall where the aorta bends) all lie within the z-range of the "looping vessel near [92,130,143] and [93,134,158]" that case 21's review notes excluded because "tracking did not establish an independent aortic origin". Two contact patches trace away from each other along the wall. No measurement we have separates it from a real hugging lumbar: strip hug ratio 1.04 to 1.08 against 1.15 on reference 23/b1 and 1.47 on 20/b1; the pair's directions 97 deg apart against 92 deg for a real adjacent pair on subject 20; origin-to-seed taper 1.8 to 1.9 against 2.0 on subject 17's real hugging vessel (matches run 0.7 to 1.4). Recorded as a known failure class (three false positives); no rule armed, because every candidate signature also fires on a real vessel in the dev set.
- **One is an intervertebral disc.** Subject 22 patch 32 (and its twin patch 35, one voxel away) points into a homogeneous grey region behind the aorta with no cortex, so the bone test cannot fire; at that case's 105 HU threshold the disc clears it. The seed's inscribed circle is 12 mm, i.e. the seed sits inside a bright region over 24 mm across. D6 rule 4 gained `blob_at_seed` (inscribed radius at the seed over 8 mm); it fires on this and on subject 25's heart and arch contacts, on no reference (the largest reference seed section is 9 mm across, inscribed about 4.5 mm; the sweep is flat from 6 to 8 mm and starts costing at 5).
- **Three of the four cut-face candidates are the terminal iliac division**, which the PDF puts outside the task and the reference checklist tells reviewers to exclude ("apply the agreed exclusion of the terminal iliac division"; case 23's notes: "the terminal iliac split near z143-147 was excluded", exactly our patch 5 at z147). Signature measured on all 25 cases: 5 mm beyond the inferior face, within 1.5 aortic radii of the axis, subjects 17, 22 and 23 show two alike lumens of 9 to 13 mm; 19, 20 and 21 show one 17 to 20 mm lumen (the aorta continuing) plus at most a 4 mm one. D6 rule 1 gained `iliac_division`: when the two lumens are there, a candidate on the inferior face with an origin over 5 mm is one of the iliacs (22 patch 24, 23 patch 5). Subject 21's 8.5 mm candidate at its cut (patch 8) is not covered, since 21's aorta has not divided 5 mm below the face; it stays flagged `near_cut_face` for organiser question 1. The fourth cut-face candidate, subject 20 patch 4 at the superior crop limit, is the "posterior track near z198-200" that case 20's notes excluded for the crop limit and uncertain 5 mm continuity; also left to question 1.
- **Rule 6 only merged survivors.** Rejecting patch 32 released patch 35 (same origin voxel, same disc). `same_origin_rejected`: a survivor on the same or an adjacent working voxel as a patch rejected for what it is (bone, blob, volume, iliac, end face) goes with it; one origin is one instance.
- **Where the remaining 13 false positives sit** (17 of 19 found, F1 0.69 at 5 mm): three are the looping vessel above; three are 2.0 to 2.4 mm origins in the borderline band (organiser question 2); two are at the cut faces (question 1); five are 2.9 to 3.2 mm vessels with no flags at lumbar and IMA positions, one of which (21 patch 7) is within 3 mm of the two structures case 21's notes list as "unresolved diameter and separate-origin versus common-trunk anatomy" (question 3).
- **Ideas measured and rejected in the same pass:** a cut-face contact fraction (the iliac candidates' patches sit on the lateral wall just above the cut, fraction 0.07 to 0.45, no separation); a seed-radius floor at 1 mm (reference 20/b5 has a 0.95 mm seed radius); the origin-to-seed taper (subject 17's real vessel at 2.0).

Findings from the instance-quality review (13 Sep, late; the 69 deg mean direction error on subject 23 and the three seeds that miss the reference daughter mask, 14 of 17 on branch):

- **Subject 23's direction error is the merged pair, not the tracer.** Reference branches 002 and 003 are two 2.2 to 2.5 mm posterior lumbars with ostia 4 mm apart whose lumina merge at the wall at 1.5 mm voxels (the recorded 23/b2 miss). Our one instance takes its ostium at b3's origin (0.96 mm), the march runs along the merged wall band into b2's course, and the seed lands on b2's mask (label 2). Matched to b3 the direction is 138 deg off; matched to b2 (4 mm, inside the cutoff) it would be 54 deg off with the seed on branch. The annotators themselves record "separate ostia versus a short common origin must be adjudicated" for this pair. Nothing in the tracer changes this; the fix would be an instance split at the wall, which was tried and rejected earlier (no neck at any prominence).
- **Subject 19's seed sits in our lumen but 2 mm below the reference's one-voxel mask.** The march stopped at 4 mm on a split (the calcified root and adjacent tract the notes mention), the straight-axis fallback has no upward component, and the vessel bends up. The seed is 0.5 mm from a branch voxel of ours, and that voxel is outside the reference label too, so snapping the seed into the lumen changes nothing. Subject 20's off-label seed is the strip-end ostium placed 3 mm along a strip whose two ends are alike (hug ratio 1.11), with a 35 deg direction error; at 1.5 mm voxels either is enough to miss a 2.5 mm vessel's mask. All 17 matched seeds lie within 0.8 mm of our own branch voxels; the three misses are the reference mask's width, not seeds in tissue.
- **Tracer stabilisation was measured and rejected.** The step log on 19 and 23 showed the march's cross-section jumping from 8 to 80 voxels and the direction turning 50 to 77 deg on alternate steps whenever the 16 mm slab touched the wall band or a neighbour. Two physically motivated remedies were implemented and swept (a lateral cap at 2 to 3 x the previous section's radius, and a per-step turn clamp of 15 to 45 deg, i.e. a 2 to 4 mm bend radius): the grid in D5 shows every combination either loses a labelled true positive (the clamp at 45 deg or tighter drops one on subject 20) or adds two to five false positives (the cap keeps alive marches that used to fall apart before 5 mm and be rejected as not followable), while the direction error stays at 21 to 22 deg because subject 23's 70 deg is the merged pair. The old tracer's fragility is doing eligibility work by accident; replacing it needs an explicit followability test first, not a smoother march. Both changes were reverted; the sweep machinery for tracing constants stays (`sweeps.py`, TRACE_SWEEPS).
- **The 8 mm blob threshold was a coin flip.** The seed section is sampled in a 16 mm window, so the inscribed radius of anything wider saturates at 7.9 to 8.1 mm; the same disc contact read 12.1 with one tracer variant and 7.98 with another. BLOB_INSCRIBED_RADIUS_MM is 7 mm (a bright region over 14 mm across at the seed; the largest reference seed section is 9 mm across).
...
- Tried and rejected 13 Sep (instance-quality review): a lateral cap on the section centroid (voxels further than TRACE_LATERAL_CAP_FACTOR x the previous section's radius from the axis dropped, on the argument that a lumen does not double in width over 1 mm) and a per-step turn clamp (TRACE_TURN_CLAMP_DEG, on the argument that no artery bends on a 2 mm radius). Grid on the five labelled cases at the 5 mm cutoff (TP / FP / mean direction / seeds on branch); the committed tracer is the first row:

  | cap factor | clamp deg | TP | FP | dir | seed |
  |---|---|---|---|---|---|
  | none | none | 17 | 13 | 21.9 | 14/17 |
  | none | 45 | 16 | 14 | 20.4 | 14/16 |
  | none | 30 | 16 | 14 | 21.5 | 14/16 |
  | 2.0 | none | 17 | 15 | 21.2 | 15/17 |
  | 2.0 | 30 | 17 | 15 | 21.7 | 15/17 |
  | 2.0 | 20 | 17 | 18 | 21.8 | 16/17 |
  | 2.5 | none | 17 | 15 | 21.0 | 16/17 |
  | 3.0 | 30 | 16 | 14 | 21.3 | 14/16 |

  A children-only cap (second blobs count as a bifurcation child only within the cap, centroid untouched) gave 17 / 14 / 21.9 / 14 at best. No setting holds every labelled case (section 3), the added false positives are marches that previously fell apart before 5 mm, and subject 23's 70 deg is the merged pair (section 1). Reverted; the `sweeps.py` TRACE_SWEEPS block keeps the evaluation path.
...
1. Cropped ends / aorta continuing past the mask, and the terminal iliac division. **Rejecting, as built.** `iliac_division`, added 13 Sep: the PDF puts the terminal division outside the task and the reference checklist tells reviewers to exclude it (case 23's notes exclude its split at exactly our candidate). Signature measured on all 25 cases: in a one-voxel slab LUMEN_PROBE_MM (5 mm, the seed distance) beyond the inferior face, within LUMEN_PROBE_LATERAL_FACTOR (1.5) aortic radii of the axis, two bright lumens both over ILIAC_LUMEN_MIN_DIAMETER_MM (6 mm; adult common iliacs are 8 to 12 mm) and alike to ILIAC_LUMEN_RATIO_MIN (0.5). Subjects 17, 22, 23 show 9 to 13 mm pairs; 19, 20, 21 one 17 to 20 mm lumen plus at most 4 mm. When the pair is there, a candidate touching the inferior face with an origin over ILIAC_CANDIDATE_MIN_DIAMETER_MM (5 mm; the only vessels that wide at the terminal aorta are the iliacs, the inferior mesenteric is 3 to 4 mm and 3 to 4 cm higher) is one of the iliacs. Removes 22 patch 24 and 23 patch 5; the continuation test already removed the larger lumen. Sweeps flat over 4 to 8 mm and 3 to 5 mm. A renal at the cut is the false-rejection risk: it would need the mask to end at the renal level with two 6 mm lumens both within 1.5 radii of the axis, which no dev case shows. The rest of rule 1 as first built: Reject if the candidate's wall patch touches an end face (its highest voxel within END_FACE_MM of the plane through the centreline endpoint with the outward tangent as normal; endpoints from frame.py, still the slice-centroid stub) AND its direction is within 20° of the outward tangent. Patch area over 40% of the aortic cross-section is a supporting signal only, not a discriminator: renal origin patches measure 150 to 240 mm² on this data, the same as the aortic cross-section. The continuation patches on subjects 19 and 20 are 440 to 460 mm² (about twice the cross-section) so a ratio above 1.5 can short-circuit the direction test. No distance-based exclusion zone (subject 3 has a real branch at the cut). This rule is exercised by subjects 17, 19 and 20, where the mask ends mid-vessel with the aorta continuing inside the image, and by subject 20 whose mask tapers to a 5 mm point at one end rather than a flat cut. It removes the continuation patch on every coarse case. Open: one further candidate per case touches the inferior face at an angle (the iliac division on 22, visible as two lumens 5 mm out; 7 to 8.5 mm vessels on 21 and 23; a 2.5 mm one on 20). They carry the `near_cut_face` flag and are kept until organiser question 1 is answered.
...
6. Duplicates. **Rejecting, as built, widened 13 Sep.** Two survivors are one branch if their ostia are within DUPLICATE_MM with directions within DUPLICATE_ANGLE_DEG, or on the same working voxel whatever their directions (one origin is one instance; a trunk that splits still has one ostium), or their traced 10 mm paths come within DUPLICATE_MM of each other (two daughters cannot share a lumen; a wall-hugging vessel leaves several contact patches along its length, e.g. subject 17). The larger wall patch wins. Genuinely separate nearby origins survive because their paths diverge. Added 13 Sep (`same_origin_rejected`): the merge only ran among survivors, so rejecting subject 22's disc contact released its twin patch on the adjacent voxel with the same 12 mm origin. A survivor whose ostium lies on the same or a 26-adjacent working voxel as a patch rejected for what it is (bone, blob, volume, iliac division, end face) now goes with it: one origin is one instance, whatever it is. Adjacent voxels rather than a distance test because the twin sat at exactly one voxel and floating point put it a hair over.
...
Status 13 Sep (evening), as built: frame.py and report.py are complete. Per case with `--report-dir`: the verification PNG (coronal and sagittal candidate projections, mask, centreline, ostia, 10 mm direction arrows), the clock map PNG (unrolled wall, markers sized by radius, green clean / amber flagged, inter-branch spacing brackets, aortic diameter panel), and one self-contained HTML (branch table with clock as h:mm, height, radius, origin diameter and flags; spacing table; both PNGs embedded; Plotly 3D with the library inlined for offline use; flag glossary; the JSON). Report generation costs 1.2 to 1.7 s and sits behind the flag; a failure in it is caught in pipeline.py and cannot cost the JSON. Flags shown per branch: `near_cut_face`, `large_patch_at_cut`, `borderline_diameter`, `no_departure`, `tangential`, `elongated_patch`, `section_merged`, `area_growth`, `axis_fallback`, `bifurcation`, radius fallbacks. Committed visual checks for cases 16, 17, 19 to 23 and 3 are in `results/visual_checks/`.

Centreline as built (frame.py): (1) the two mask voxels geodesically furthest apart, found by a double sweep of `skimage.graph.MCP_Geometric` on a 2x downsampled mask (same endpoints as full resolution on the longest case, 6x faster); (2) the minimum-cost path between them at working resolution with cost (inside distance)^-2, which keeps the path on the axis through the S-bends of subjects 3 and 6 where the slice-centroid line drifts (checked on projections of 3, 6, 18, 20, 21, 25); (3) the rim-to-axis stretch at each end (about one radius) is cut and replaced by the centroid of the end face, i.e. the boundary voxels beyond the cut whose outward normal is within 60 deg of the outward tangent, within 1.5 radii of the axis; this handles a flat cut, an oblique cut, a tapered end (subject 20) and a mask that reaches the image boundary (the inside distance transform is padded so those faces exist); (4) 1 mm resampling and a 3 mm Gaussian along the arc with the endpoints pinned. The end-face tangent for rule 1 is the chord over the first radius. The inside distance transform is padded by one voxel so a mask cut by the image boundary still has faces. Fallback to slice centroids if any step raises. 0.2 to 3 s per case.

Clock frame as built, a change from the first draft: 12 o'clock at every centreline point is the patient's anterior projected into the plane perpendicular to the local tangent, and 3 o'clock is that vector crossed with the tangent (patient's left on a vertical aorta). The first draft transported the anterior vector from the inferior end with a rotation-minimising frame. Both are measured in the perpendicular plane, so both fix the tilt error of a naive axial angle, and they coincide on a planar curve (tested to 1e-6 on a 60 mm-radius bend). They differ where the centreline has torsion: on subject 3 (S-bend plus forward bow) the transported vector ends 43 deg from the patient's anterior and on subject 6 35 deg, so a lumbar at the patient's true 6 o'clock would have been reported at 4:30 or 7:30. A surgeon means the patient's anterior at each level, so the projection is the reported convention; the transported frame is still computed and its maximum deviation is logged as `twist_deg` (0 to 10 deg on the straight coarse cases), and it takes over only where the tangent runs antero-posteriorly and the projection vanishes (`CLOCK_ANTERIOR_MIN_SIN`; the arch on subject 25). The local aortic radius along the line is the median lateral distance to lateral-wall boundary voxels in a 1 mm slab (not the inside distance, which measures the face near a cut).
...
- Centreline method (chosen after subject 3; built as described above): slice centroids fail on a tortuous aorta because an axial slice can cut it obliquely or twice. Use a geodesic path: take the two mask voxels furthest apart along the mask as endpoints, then find the minimum-cost path between them through the mask interior with cost inversely related to distance-from-boundary, so the path stays central. Smooth the result. Fallback: slice centroids. The centreline endpoints also define the end faces used by D2 and D6 rule 1, so frame.py's endpoint step is shared with the engine even though the rest of the frame is display-only.
- Clock convention: 12 = anterior, 3 = patient's left, 6 = posterior, 9 = patient's right. Angle measured in the plane perpendicular to the local centreline tangent. Not optional: subjects 2 and 3 curve enough that a naive axial angle would be wrong by tens of degrees. First draft: a rotation-minimising frame transported from the inferior end; as built: the patient's anterior projected at each level, with the transported frame as a diagnostic (see the paragraph above for the measurement that decided it). On a whole-aorta case (subject 25) the projection is undefined through the arch and the transported frame takes over there; that case is out of scope and nothing is adjusted for it.
...
branchseed/
  ...
  tracing.py        D5
  filters.py        D6
  frame.py          geodesic centreline, end faces, anatomical clock frame, mm -> (height, clock), aortic radius profile, spacing
  report.py         D8 HTML report (table, flags, spacing, Plotly 3D), clock map PNG, verification PNG
  ...
  tests/            one fake-data test per module (99 tests) plus real-data tests on subjects 19 and 24
  results/          dev_scores.txt, dev_scores_skeleton_baseline.txt, reference_ledger.txt, sweeps.txt, invariants.txt, predictions/ (all 25 dev cases), visual_checks/ (PNGs for 8 cases)
...
- frame: centreline points (mm, 1 mm apart, superior first), tangents, per-point 12 and 3 o'clock vectors, arc length, endpoints and end faces, local aortic radius, function mm -> (height, clock), inter-branch spacing.
...
- Centreline and frame: 0.2 to 3 s (as built: 3 s on the 500 mm subject 25).
- Report: 1.2 to 1.7 s as built, behind a flag.
...
- Switched off with the measurement recorded next to the switch, to be re-armed only on new evidence: the D4 axis refinement (`OSTIUM_USE_AXIS_REFINEMENT`), rule 3 departure, rule 4 area growth (bowel cases 8 and 12 checked: nothing for it to remove). The sweeps in `results/sweeps.txt` show where a constant would score better on the dev set and the physical value was kept anyway (MIN_ORIGIN_DIAMETER 2.5 and REGION_VOLUME_CAP 0.75 each drop 5 to 6 false positives at no labelled cost).
...
Status at the end of 13 Sep: days 1 and 2 are done (engine complete through D6, reviewed by eye, scored, invariants clean on all 25 cases, runtime under 10 s per case), and of the day-3 list items (1) frame and (2) report are built, with the dev-set predictions and the visual checks committed under `results/`. Remaining, in order: (3) submission mechanics that wait on the organisers (vendored wheels once the platform is known, offline install test); (4) rule 4 area growth: looked at on 13 Sep, stays a flag (subject 12 flags nothing, subject 8's two hits are already rejected as duplicate and bone; see config.AREA_GROWTH_MAX); (5) demo material from the ledger, gallery and reports. Organiser answers (section 6) may reopen rule 1 at the inferior cut and the borderline-diameter handling. See `docs/HANDOFF.md`.
...
Read from the reference package on 13 Sep (evening), while the answers are pending: the reviewer checklist instructs reviewers to "exclude flat superior/inferior crop caps as origins" and to "apply the agreed exclusion of the terminal iliac division", and every cap-adjacent candidate the case notes mention was excluded or left unresolved (case 20's posterior tracks at the superior crop limit, case 21's posterior structure at or below the inferior cap, case 23's iliac split), which is most of question 1: the iliac division is out by policy (now rule 1), and vessels starting on the cap are at best unresolved. For question 2 the checklist says how unresolved branches are handled in scoring is to be "specified before release", so it is undecided on their side too. For question 3 the notes name the unresolved structures per case (21: two near [98,123,154] and [100,124,155]; 19: posterior tracks at z136-138; 22: one excluded candidate); one of our clean false positives coincides with 21's pair, the rest are not listed either way. Question 4's second half is answered by the README (the supplied mask boundary is the origin boundary); the cutoff, questions 5 and 6 are not in the package.
