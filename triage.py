...
def read_any(path):
    ...
    try:
        ...
        stats_extra["header_rotation_deg"] = round(
            float(np.degrees(np.arccos((np.trace(R) - 1) / 2))), 2
        )
        return im
...
full_shape = ct.shape
stats_pre = {
    "full_size_zyx": list(full_shape),
    "mask_touches_z_boundary_full": [bool(m[0].any()), bool(m[-1].any())],
}
pad_vox = [int(np.ceil(30.0 / s)) for s in sp]
obj = ndimage.find_objects(m.astype(np.int8))[0]
crop = tuple(
    slice(max(0, o.start - pv), min(n, o.stop + pv))
    for o, pv, n in zip(obj, pad_vox, full_shape)
)
ct = np.ascontiguousarray(ct[crop])
m = np.ascontiguousarray(m[crop])
ISO = 0.8
...
if abs(max(sp) - ISO) > 0.05 or abs(min(sp) - ISO) > 0.05:
    ...
    cimg = sitk.GetImageFromArray(ct)
    cimg.SetSpacing(img.GetSpacing())
    cimg.SetDirection(img.GetDirection())
    cimg.SetOrigin(img.TransformIndexToPhysicalPoint(origin_idx))
    cm = sitk.GetImageFromArray(m.astype(np.uint8))
    cm.CopyInformation(cimg)
    newsize = [
        int(round(sz * s_ / ISO)) for sz, s_ in zip(cimg.GetSize(), cimg.GetSpacing())
    ]
    ...
    ct = sitk.GetArrayFromImage(
        sitk.Resample(
            cimg, ref, sitk.Transform(), sitk.sitkLinear, -1000.0, sitk.sitkFloat32
        )
    )
    m = (
        sitk.GetArrayFromImage(
            sitk.Resample(
                cm, ref, sitk.Transform(), sitk.sitkNearestNeighbor, 0, sitk.sitkUInt8
            )
        )
        > 0
    )
...
stats["mask_unique_values"] = [
    int(v) for v in np.unique(sitk.GetArrayFromImage(msk))[:5]
]
...
stats["equiv_diam_mm_min_med_max"] = [
    round(float(diam.min()), 1),
    round(float(np.median(diam)), 1),
    round(float(diam.max()), 1),
]
...
st.flat[np.where((((idx - np.array(r)) / np.maximum(r, 1)) ** 2).sum(1) <= 1.0)[0]] = (
    True
)
...
stats["wall_patch_areas_mm2_top8"] = sorted(
    [round(float(x), 1) for x in sizes if x > 5], reverse=True
)[:8]
# bone-sized blobs in the shell
cl, nc = ndimage.label(cand_open)
vols = (
    ndimage.sum(cand_open, cl, range(1, nc + 1)) * np.prod(sp) / 1000
    if nc
    else np.array([])
)
stats["aorta_xsec_mm2_median"] = round(float(np.median(areas)), 1)
stats["largest_patch_over_40pct_aorta"] = bool(
    len(sizes) and sizes.max() > 0.4 * np.median(areas)
)
...
for name, z, step in (("top", zs[-1], 1), ("bottom", zs[0], -1)):
    ...
    if 0 <= zz < ct.shape[0]:
        sl_m = m[z]
        stats[f"bright_beyond_{name}_face_fraction"] = round(
            float(bright[zz][sl_m].mean()), 3
        )
...
axes[0].imshow(
    np.ma.masked_where(mm == 0, mm), cmap="spring", alpha=0.35, aspect=asp_cor
)
...
axes[1].imshow(
    np.ma.masked_where(ms == 0, ms), cmap="spring", alpha=0.35, aspect=asp_sag
)
