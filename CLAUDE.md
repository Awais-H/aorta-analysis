...
Engine complete through D6 and reviewed by eye (gallery.py) on subjects 16, 17, 21, 22; frame (geodesic centreline, anatomical clock) and report (clock map, flags, Plotly 3D, verification PNG) built. On cases 19-23 at 5 mm: TP 17 / FP 16 / FN 2, F1 0.65, ostium 1.35 mm, seed on branch 14/17; invariants clean on all 25, under 10 s per case. Dev-set predictions in results/predictions, visual checks in results/visual_checks. Rule 4 area growth reviewed on subjects 8 and 12 and kept as a flag. Next in order: vendored wheels and offline install test (needs the organisers' OS/Python answer), demo material. Six questions are with the organisers (SPEC section 6); their answers may reopen rule 1 at the inferior cut and the borderline-diameter handling. Code is on the fork nikolakr7/aorta-analysis, branch skeleton.
