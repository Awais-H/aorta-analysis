...
Engine complete through D6: candidates, instances, ostium (inscribed circle; axis refinement off by default), tracing (march + axis fallback, chord direction, 0.25 mm plane radius), filters (rules 1, 2, 3, 4 volume, 6, 7; area growth and rule 3 secondaries are flags pending the bowel cases). Scorer: score, ledger, invariants; sweeps.py runs the section 3 sensitivity sweeps. On cases 19-23 at 5 mm: TP 14 / FP 26 / FN 5, F1 0.47, ostium 1.26 mm, seed on branch 12/14. Known misses: 19/b2 (1-voxel patch), 20/b4 (shallow-angle origin, march wanders), 20/b1 and 23/b1 (root merged into the continuation patch) and 23/b2+b3 (one patch, two origins): the last three are D3 work, next. Remaining stubs: frame (slice centroids), report (PNG + table).
