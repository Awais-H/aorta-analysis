...
Ostium (D4) and tracing (D5) are real: inscribed-circle ostium snapped to the mask boundary (axis-intersection refinement implemented but off by default, see config.OSTIUM_USE_AXIS_REFINEMENT), slice-and-centroid march through the raw shell with bifurcation stop and axis fallback, chord direction, area-equivalent radius on a 0.25 mm plane. Scorer matching is cutoff-aware. results/dev_scores.txt and results/invariants.txt are current; results/dev_scores_skeleton_baseline.txt is the pre-D4/D5 baseline rescored with the same scorer. Remaining stubs: filters (eligibility rule only: precision 0.15 on cases 19-23, 91 false positives), frame (slice centroids), report (PNG + table). Next per SPEC section 5: filters rules 1, 3, 4, 6, 7.
