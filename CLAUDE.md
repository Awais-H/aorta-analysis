...
Engine complete through D6 and reviewed by eye (gallery.py) on subjects 16, 17, 21, 22; frame (geodesic centreline, anatomical clock) and report (clock map, flags, Plotly 3D, verification PNG) built. On cases 19-23 at 5 mm: TP 17 / FP 12 / FN 2, F1 0.71, ostium 1.35 mm, seed on branch 14/17 (rule 1 iliac division, rule 4 blob at seed, rule 6 same-origin added after the false-positive review, rule 2 path_at_image_edge after the cut-face review, SPEC section 1); invariants run on all 25 (no crash, budget held; sub-1 mm radii and under-threshold seeds on a few coarse cases are recorded), under 7 s and 1.3 GB peak per case; leave-one-out (results/leave_one_out.txt) shows the physical constants are the held-out optimum. The perturbation check (perturb_check.py, results/perturbation_check.txt) found the D1 threshold fragile to a tighter mask (std term); recorded for a decision, not fixed. Dev-set predictions in results/predictions, visual checks in results/visual_checks. Rule 4 area growth reviewed on subjects 8 and 12 and kept as a flag. The five unflagged false positives were adjudicated by eye on 13 Sep (review_dense.py; SPEC section 1): four real unannotated lumbars kept, one passing contact of an already-reported daughter recorded without a rule. The 1.5 mm check on subjects 1 to 15 (coarse_check.py, results/coarse_check.txt) shows 66 of 80 native branches surviving coarsening with the same 1.36 mm ostium error as the references; losses are erased or merged small origins, gains are partial-volume bridges; nothing tuned to it. Next in order: perturbation-stability check and the two cut-face candidates (docs/HANDOFF.md), vendored wheels and offline install test (needs the organisers' OS/Python answer), demo material. Six questions are with the organisers (SPEC section 6); their answers may reopen rule 1 at the inferior cut and the borderline-diameter handling. Code is on the fork nikolakr7/aorta-analysis: main is current through 13 Sep, work continues on precision-review.
