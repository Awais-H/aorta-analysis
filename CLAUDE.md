...
Engine complete through D6. D4 handles the hugging branch (elongated patch: ostium at the end that hugs the wall, tracer along the strip). D6 rule 3 (path departure) is a flag, not a rejection: wall-hugging lumbars and the IMA are real daughters that never leave the wall within 10 mm (references 20/b1, 23/b1; subject 2's IMA), and no vein in the dev set clears the threshold. On cases 19-23 at 5 mm: TP 17 / FP 26 / FN 2, F1 0.55, ostium 1.35 mm, seed on branch 14/17. Known misses: 19/b2 (1-voxel patch, rule 7 noise floor), 23/b2 (merged with b3 at the wall at 1.5 mm voxels: resolution limit). Remaining stubs: frame (slice centroids), report (PNG + table). Axis refinement and area growth are switched off with the measurements recorded in config and filters.
